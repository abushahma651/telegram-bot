...
async def main():
    ...
    print(f"{Fore.CYAN}📱 BY @CyberHacked0 @hackedanu | @nothing_anu hacked.aPHONE ANURAGXNOTHING{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}🚨 WARNING: THIS WILL COMPLETELY DESTROY TARGET PHONE!{Style.RESET_ALL}")
    
    phone = input(f"\n{Fore.YELLOW}BY ANURAGXNOTHING 🎯 Enter target number (10 digits): {Style.RESET_ALL}")
    
    if not phone.isdigit() or len(phone) != 10:
        print(f"{Fore.RED}❌ Invalid number!{Style.RESET_ALL}")
        return
    
    print(f"{Fore.RED}🚨 TARGET LOCKED BY NOTHING: +91{phone}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}💣 READY WITH {len(ULTIMATE_APIS)} BOMBING APIS{Style.RESET_ALL}")
    
    confirm = input(f"{Fore.RED} ANURAGXANU 💀 ACTIVATE ULTIMATE DESTROYER? (y/n): {Style.RESET_ALL}").lower()
